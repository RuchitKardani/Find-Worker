<?php
// session_start();
include '_nav.php'; //here we do not required the session because it alredy declared in nav.php file
...
if (isset($_POST["submit"])) {
    ...
    $new = $_POST["new"];
    $repass = $_POST["repass"];
    $res = mysqli_query($con, "SELECT * FROM servicetaker WHERE email='" . $uname . "' and Password='" . $old . "'");
    if (mysqli_num_rows($res)) {
        if ($new == $repass)
            mysqli_query($con, "UPDATE servicetaker SET Password='" . $new . "' WHERE email='" . $uname . "'");
    } else {
