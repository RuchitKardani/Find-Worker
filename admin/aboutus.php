...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES["service_taker_image"]) && $_FILES["service_taker_image"]["name"] !== "") {
        // Handle the service taker side image update
        $target_dir_taker = "about_us_img/";
        $target_file_taker = $target_dir_taker . basename($_FILES["service_taker_image"]["name"]);
        move_uploaded_file($_FILES["service_taker_image"]["tmp_name"], $target_file_taker);
        $updateImageTaker = "service_taker_image = '$target_file_taker',";
    
    }

    if (isset($_FILES["service_provider_image"]) && $_FILES["service_provider_image"]["name"] !== "") {
        // Handle the service provider side image update
        $target_dir_provider = "about_us_img/";
        $target_file_provider = $target_dir_provider . basename($_FILES["service_provider_image"]["name"]);
        move_uploaded_file($_FILES["service_provider_image"]["tmp_name"], $target_file_provider);
        $updateImageProvider = "provider_image = '$target_file_provider',";
    }
                ...
                <div class="card">
                    ...
                    <form method="post">
                        <div class="form-group">
                            <label for="service_taker_image">Service Taker Image:</label>
                            <img src="about_us_img/<?php echo $data['service_taker_image']; ?>" class="img-thumbnail" style="max-height: 200px;">
                            <input type="file" class="form-control-file" id="service_taker_image" name="service_taker_image">
                        </div>

                        <div class="form-group">
                            <label for="service_provider_image">Service Provider Image:</label>
                            <img src="<?php echo $data['provider_image']; ?>" class="img-thumbnail" style="max-height: 200px;">
                            <input type="file" class="form-control-file" id="service_provider_image" name="service_provider_image">
                        </div>
